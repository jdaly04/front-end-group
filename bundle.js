...
/***/ function(module, exports, __webpack_require__) {
	...
	var backEndEvents = __webpack_require__(8);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var signInSuccess = function signInSuccess(data) {
	  ...
	  $('#signUpBox').empty().append("User ID:", +data.user.id);
	  $('#close').hide();
	  $('#sign-up').hide();
	  $('#sign-in').hide();
	  $('#needAccountBox').hide();
	  ...
	  $('#signInBox').show();
	  $('.options').empty().append("User ID:", +data.user.id);
	  $('.userIdMain').empty().append("User ID:", +data.user.id);
	  $('#viewAllImages').show();
	  $('#myModal1').modal('hide');
	  $('#outANDin').hide();
	  $('#imageOptions').show();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onPostImage = function onPostImage(event) {
	  event.preventDefault();
	  var postImage = getFormFields(event.target);
	  api.postImage(postImage).then(ui.postImageSuccess).catch(ui.postImageFailure);
	};

	var onDeleteImage = function onDeleteImage(event) {
	  event.preventDefault();
	  var deleteImage = getFormFields(event.target);
	  api.deleteImage(deleteImage.id).then(ui.deleteImageSuccess).catch(ui.deleteImageFailure);
	};

	var onUpdateImage = function onUpdateImage(event) {
	  event.preventDefault();
	  var updateImage = getFormFields(event.target);
	  api.updateImage(updateImage).then(ui.updateImageSuccess).catch(ui.updateImageFailure);
	};

	var onViewImage = function onViewImage(event) {
	  event.preventDefault();
	  var viewImage = getFormFields(event.target);
	  api.viewImage(viewImage).then(ui.viewImageSuccess).catch(ui.viewImageFailure);
	};

	var onViewAllImages = function onViewAllImages(event) {
	  event.preventDefault();
	  var viewAllImages = getFormFields(event.target);
	  api.viewAllImages(viewAllImages).then(ui.viewAllImagesSuccess).catch(ui.viewAllImagesFailure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var postImage = function postImage(data) {
	  return $.ajax({
	...
	var viewImage = function viewImage(data) {
	  return $.ajax({
	    url: store.host + '/images/' + data.image.id,
	    method: 'GET',
	    headers: {
	...
	var viewAllImages = function viewAllImages(data) {
	  return $.ajax({
	    url: store.host + '/images/',
	    method: 'GET',
	    // headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(6);
	var showImagesTemplate = __webpack_require__(11);
	var showHoverTemplate = __webpack_require__(31);
	...
	var viewImageSuccess = function viewImageSuccess(data) {
	  var img = $("<img>").attr("src", data.image.url).height(300).width(300);
	  $(".img-container").empty().show().append(img);
	  $('.img-container-info').empty().show().append("Image ID:", +data.image.id);
	  $('.img-container-info-one').hide();
	  $('.img-container-info-two').empty().show().append("User ID:", +data.image.user_id);
	  $('#signInBox').empty().append('Go to The Main Page to Take a Look');
	  $('#myModal1').modal('hide');
	  $('#viewImage').trigger('reset');
	};

	var viewImageFailure = function viewImageFailure(data) {
	  console.log(data);
	  $('#signInBox').empty().append('Maybe Not a Valid #?  Check the List!  Hover for Info!');
	};

	var postImageSuccess = function postImageSuccess(data) {
	  console.log(data);
	  var img = $("<img>").attr("src", data.image.url).height(300).width(300);
	  $(".img-container").empty().show().append(img);
	  $('.img-container-info').empty().show().append("Image ID:", +data.image.id);
	  $('.img-container-info-one').empty().show().append('Image Was Just Posted!');
	  $('.img-container-info-two').empty().show().append("User ID:", +data.image.user_id);
	  $('#signInBox').empty().append('Image Was Posted As ID:', +data.image.id);
	  $('#myModal1').modal('hide');
	  $('#postImage').trigger('reset');
	};

	var postImageFailure = function postImageFailure() {
	...
	var deleteImageSuccess = function deleteImageSuccess() {
	  ...
	  $(".img-container").empty();
	  $('.img-container-info').empty().text("Image Was Deleted!");
	  $('.img-container-info-one').empty();
	  $('.img-container-info-two').empty();
	  $('#deleteImage').trigger('reset');
	};

	var deleteImageFailure = function deleteImageFailure(data) {
	  console.log(data);
	  $('#signInBox').empty().append('That May Not Belong To You (Or It May Not Exist)');
	};

	var updateImageSuccess = function updateImageSuccess(data) {
	  console.log(data);
	  $('#signInBox').empty().append('Image URL Was Updated');
	  $('#updateImage').trigger('reset');
	};

	var updateImageFailure = function updateImageFailure(data) {
	  console.log(data);
	  $('#signInBox').empty().append('Nope.  Something Went Wrong.');
	};

	var viewAllImagesFailure = function viewAllImagesFailure(data) {
	  console.log(data);
	  $('#signInBox').empty();
	};

	var viewAllImagesSuccess = function viewAllImagesSuccess(data) {
	  ...
	  data.images.forEach(function (e, i) {
	    return data.images[i].title = 'Image ID: ' + e.id + ' URL: ' + e.url + ' User ID: ' + e.user_id;
	  });
	...
	module.exports = {
	  failure: failure,
	  success: success,
	  postImageSuccess: postImageSuccess,
	  postImageFailure: postImageFailure,
	  deleteImageSuccess: deleteImageSuccess,
	  deleteImageFailure: deleteImageFailure,
	  updateImageSuccess: updateImageSuccess,
	  updateImageFailure: updateImageFailure,
	  viewImageSuccess: viewImageSuccess,
	  viewImageFailure: viewImageFailure,
	  viewAllImagesSuccess: viewAllImagesSuccess,
	  viewAllImagesFailure: viewAllImagesFailure
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n\n<div class='imageImage pic'>\n\n<img src=\""
	    ...
	    + "\" width=\"100%\" height=\"100%\" alt=\"There is No Image, ya dummy\" />\n\n</div>\n\n\n\n\n</a>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.images : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
